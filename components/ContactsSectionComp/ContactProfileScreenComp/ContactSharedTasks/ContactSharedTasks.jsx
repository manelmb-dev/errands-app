...
const ContactSharedTasks = ({ currentContact }) => {
  ...
  const [mainTab, setMainTab] = useState("all"); // all | outgoing | incoming
  ...
  const contactSharedErrands = useMemo(() => {
  ...
  const filteredErrands = useMemo(() => {
    const baseList = contactSharedErrands.filter((errand) => {
      ...
      if (mainTab === "outgoing" && !isSender) return false;
      if (mainTab === "incoming" && !isReceiver) return false;

      if (subFilter === "pending" && isCompleted) return false;
      if (subFilter === "completed" && !isCompleted) return false;
  ...
  }, [contactSharedErrands, currentContact, mainTab, subFilter]);
  ...
  return (
    <View>
      <Text
        className={`pb-3 text-2xl font-semibold text-[${themes[theme].text}] text-center`}
      >
        {i18n.t("sharedErrands")}
      </Text>
      {/* Tabs */}
      <View className="flex-row justify-center items-center gap-4 px-2 pb-4">
...
export default ContactSharedTasks;
